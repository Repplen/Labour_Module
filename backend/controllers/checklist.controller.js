...
const {
  applyPermanentTransferContext: applyChecklistTransferPermanentContext,
  applyTemporaryTransferContext: applyChecklistTransferTemporaryContext,
  loadPermanentTransferContext: loadChecklistTransferPermanentContext,
  loadTemporaryTransferContext: loadChecklistTransferTemporaryContext,
...
const approveChecklistAdminRequestRecord = async ({ request, reviewer, remarks = "" }) => {
  ...
  if (request.moduleKey === CHECKLIST_REQUEST_MODULE_KEYS.checklistMaster) {
    ...
    if (request.actionType === CHECKLIST_REQUEST_ACTIONS.permanentTransfer) {
      const transferContext = await loadChecklistTransferPermanentContext({
      ...
      const transferResult = await applyChecklistTransferPermanentContext({
        actorUser,
      ...
      const transferContext = await loadChecklistTransferTemporaryContext({
        body: request.requestPayload,
      });
      ...
      const transferResult = await applyChecklistTransferTemporaryContext({
        actorUser,
        context: transferContext.payload,
      });
      resultEntryId = transferResult?.history?._id || null;
